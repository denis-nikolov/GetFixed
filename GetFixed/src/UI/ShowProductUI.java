...
public class ShowProductUI {
	...
	ShowProductUI(JPanel contentPanel, JPanel secondaryMenuPanel,
	...
	void make() {
		...
		table.setModel(new DefaultTableModel(new Object[][] { { null, null,
				...
				null } }, new String[] { "Barcode", "Name", "Selling price",
				"Leasing price", "Ordering price", "Price discount",
				"Amount discount", "Amount", "Min amount", "Max amount",
				...
				if (rdbtnAalborg.isSelected()) {
					...
					for (int rows = 0; rows < table.getRowCount(); rows++) {
						...
						for (int columns = 0; columns < table.getColumnCount(); columns++) {
							values.add(table.getValueAt(rows, columns)
									.toString());
						}
		...
		btnSearch.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				if (flag) {
					model.addRow(productCtr.addProductByBarcode(Integer
							.parseInt(search)));
				} else {
		...
		btnUpdate.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				for (int i : vals) {
					...
					productCtr.updateProduct(Integer.parseInt(values.get(0)),
							values.get(1), Double.parseDouble(values.get(2)),
				...
				for (int i : vals) {
					...
					try {
						Product product = productCtr
								.findByBarcodeAndDepartmentLocation(Integer
										.parseInt(table.getValueAt(i, 0)
												.toString()),
